...
int main(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		if (ip_valid(dg[iget].dg_data))
		{
			if (!get_info_ip_address(dg[iget].dg_data))
			{
				strcpy(dg[iget].dg_data, "Not found information");
			}
		}
		else
		{
			if (!get_info_domain(dg[iget].dg_data))
			{
				strcpy(dg[iget].dg_data, "Not found information");
			}
		}
		sendto(sockfd, dg[iget].dg_data, strlen(dg[iget].dg_data), 0,
